...
clean_orgs_level <- clean_orgs %>%
  left_join(keph_level, by = c("id" = "id"), multiple = "first") 

clean_orgs_level %>% filter(!is.na(Facility_Level)) %>% group_by(level) %>% tally()


fwrite(clean_orgs_level, paste0("data/kenya-hpt/metadata/orgunits.csv"))
fwrite(clean_orgs_level, "G:\\.shortcut-targets-by-id\\1-I-AM6fIfll60idw-jnzME08IVKzNdfj\\MLE_DATA\\Dashboards\\HPT\\metadata\\orgunits.csv")

clean_orgs_level %>%
  filter(id == "pZRr7UTYBCJ")
